...
<?php 
	...
	if($_POST["credential_insert_method"] == "select")
		...
		do_query("INSERT into coffee_cred_db.credential_master_list(name) values('$credential_name')");
		$credential_id = mysqli_insert_id(get_sql_conn());
		do_query("INSERT into coffee_cred_db.credential_category_connection(categoryID, credentialID) 
	...
	if ($_POST["evidence_method"] == "upload"){
		...
		if (move_uploaded_file($_FILES["evidence_file"]["tmp_name"], $evidence_filedir)){
			$credential_insertion_query = do_query("
										INSERT into coffee_cred_db.user_credentials
